...
class PythonTestGenerator(TestTranslator):
    ...
    def _generate_qontinui_imports(self, test_file: TestFile) -> list[str]:
        ...
        imports.append("from qontinui_devtools.core import QontinuiCore")

        # Add mock imports if needed
        if test_file.mock_usage:
            imports.append(
                "from qontinui_devtools.test_migration.mocks import QontinuiMockGenerator"
            )

        # Add integration test imports for integration tests
        if test_file.test_type == TestType.INTEGRATION:
            imports.append("from qontinui_devtools.config import ConfigurationManager")
            imports.append("from qontinui_devtools.startup import QontinuiStartup")
